/*
Project - M
1st Open Source : Mariko
© 2018 Dark Tornado, All rights reserved.

<작동 방식>
1. 채팅방에서 사람들이 하는 채팅을 '내장메모리/Mariko/' 폴더에 '방이름.txt' 파일로 저장.
  -> 각 채팅들은 엔터로 구분합니다. 따라서, 엔터가 포함된 채팅은 저장하지 않습니다.
  -> 학습 및 채팅 전송은 채팅방마다 따로따로 작동합니다.
2. 해당 채팅방에서 채팅이 수신되면 10% 확률로 수신된 채팅과 파일 안에 있는 채팅의 유사도를 굉장히 대충 검사.
...
- 소스 사용시 원작자를 밝혀주세요.
  ex) 이 봇은 Dark Tornado의 Project M - Mariko 소스를 사용하였습니다.
...
Mariko.getReply = function(room, msg) { //수신된 채팅에 대한 적당한 답변 반환
    ...
    if (data != null && Math.floor(Math.random() * 10) == 0) { //저장된 채팅이 없거나, 10% 확률이 터진게 아니면, 작동 안함
        ...
        for (var n = 0; n < data.length - 1; n++) { //적당하다 싶은 녀석들을
            if (Mariko.checkWord(data[n], msg)) result.push(data[n + 1]); //배열에 추가
        }
...
Mariko.say = function(msg, replier) { //그냥 말하는 함수
    replier.reply("[AI] " + msg); //앞에다가 이상한 문구 붙이는 용도
};
Mariko.isValidData = function(msg) { //배울 만한 채팅인지 구분하는 함수
...
DB.createDir = function() { //배운 채팅들이 저장될 폴더를 만드는 함수
    var folder = new java.io.File(sdcard + "/Mariko/"); //File 인스턴스 생성
    folder.mkdirs(); //폴더 생성
};
DB.saveData = function(name, msg) { //파일에 내용을 저장하는 함수
    try { //사실, 나도 어디서 긁어와서 이곳저곳에서 사용하는 거임
        var file = new java.io.File(sdcard + "/Mariko/" + name + ".txt");
...
function response(room, msg, sender, isGroupChat, replier) {
    ...
    var chat = Mariko.getReply(room, msg);
    if (chat != null) Mariko.say(chat, replier);

    /*채팅을 학습하는 부분*/
    if (Mariko.isValidData(msg)) { //배울 만한 채팅인 경우,
        var data = DB.readData(room); //배운 채팅 목록을 가져옴
        if (data == null) { //이미 배운게 없다면
