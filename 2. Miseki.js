/*
Project - M
2nd Open Source : Miseki
© 2018 Dark Tornado, All rights reserved.

<작동 방식>
1. 채팅방에서 사람들이 하는 채팅을 '내장메모리/Miseki/' 폴더에 '방이름.txt' 파일로 저장.
...
2. 해당 채팅방에서 채팅이 수신되면 10% 확률로 수신된 채팅과 파일 안에 있는 채팅의 유사도를 대충 검사.
  -> 어절이 두 개 이상 일치하면 유사하다고 판단하지만, 50% 확률로 하나만 일치해도 유사하다고 판단.
...
- 소스 사용시 원작자를 밝혀주세요.
  ex) 이 봇은 Dark Tornado의 Project M - Miseki 소스를 사용하였습니다.
...
Miseki.say = function(msg, replier) { //그냥 말하는 함수
...
DB.createDir = function() { //배운 채팅들이 저장될 폴더를 만드는 함수
    var folder = new java.io.File(sdcard + "/Miseki/"); //File 인스턴스 생성
    folder.mkdirs(); //폴더 생성
};
DB.saveData = function(name, msg) { //파일에 내용을 저장하는 함수
    try { //사실, 나도 어디서 긁어와서 이곳저곳에서 사용하는 거임
        var file = new java.io.File(sdcard + "/Miseki/" + name + ".txt");
...
function response(room, msg, sender, isGroupChat, replier) {
    ...
    if (chat != null) Miseki.say(chat, replier);
